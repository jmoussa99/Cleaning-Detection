...
ax2.axhline(y=0.82, color='r', linestyle='--', label='Decision Threshold (0.82)')
if df['is_cleaning'].sum() > 0:
...
output_df = output_df[output_df['reconstruction_error'] < 0.31].drop_duplicates()

# Save high-probability cleaning events
high_prob_cleaning = output_df[output_df['cleaning_probability'] > 0.82].sort_values(
    'cleaning_probability', ascending=False)

print(f"\n{len(high_prob_cleaning)} cycles identified with >82% cleaning probability")
