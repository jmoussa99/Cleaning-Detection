...
def main():
    # File paths
    jsonl_file = "events_2025-10-27_to_2025-11-20.jsonl"
    csv_file = "nose_cap_14-247-labeled.csv"
    
    print("="*70)
    print("ADDING EVENTS FROM JSONL TO CSV")
    print("="*70)
    
    # Step 1: Process JSONL file using JSONL_DataProcessor
    print("\n" + "="*50)
    print("PROCESSING JSONL FILE")
    print("="*50)
    data_processor = JSONL_DataProcessor(jsonl_file)
    
    # Step 2: Extract features using FeatureEngineering
    print("\n" + "="*50)
    ...
    # Step 3: Read existing CSV
    print(f"\nReading existing CSV file: {csv_file}")
    try:
    ...
    # Step 4: Merge dataframes (removing duplicates)
    merged_df = merge_dataframes(all_features, existing_df)
    
    # Step 5: Save merged dataframe
    print(f"\nSaving merged data to: {csv_file}")
    merged_df.to_csv(csv_file, index=False)
    print(f"✅ Successfully saved {len(merged_df)} rows to {csv_file}")
    
    print("\n" + "="*70)
    print("SUCCESS! Events added to CSV without duplicates")
    print("="*70)
