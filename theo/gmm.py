...
def preprocess_data(df, use_differences=True):
    ...
    -----------
    df : DataFrame
        Input dataframe with FFT and scalar features
    use_differences : bool
        If True, use temporal differences (change from previous sample) instead of raw values.
        This captures the rate of change which is more characteristic of cleaning events.
    """
    # Parse array columns
    ...
    # Add FFT features (raw or differences)
    for col in array_columns:
        arr = np.array(df[col].tolist())
        
        if use_differences:
            # Compute temporal differences (current - previous)
            # First row gets 0 difference
            arr_diff = np.diff(arr, axis=0, prepend=arr[0:1])
            
            for i in range(arr_diff.shape[1]):
                features_list.append(arr_diff[:, i])
                feature_names.append(f'{col}_diff_{i}')
        else:
            # Use raw FFT magnitudes
    ...
    for col in scalar_features:
        if col in df.columns:
            vals = df[col].values
            if use_differences:
                # Compute temporal differences for scalar features too
                vals_diff = np.diff(vals, prepend=vals[0])
                features_list.append(vals_diff)
...
def extract_cleaning_windows(df, window_size=45, include_cleaning_rows=True):
    """
    Extract full feature data from windows around cleaning events.
    Returns data from window_size rows before cleaning_start, the cleaning rows themselves,
    and window_size rows after cleaning_end.
    ...
    window_size : int
        Number of rows before/after cleaning to include
    include_cleaning_rows : bool
        If True, include the actual cleaning_start to cleaning_end rows in training
    """
    print("\n" + "="*80)
    ...
    print(f"Include cleaning rows in training: {include_cleaning_rows}")
    ...
    for i, (start_idx, end_idx) in enumerate(zip(cleaning_starts, cleaning_ends)):
        ...
        # Extract the cleaning rows themselves (from start to end, inclusive)
        if include_cleaning_rows:
            cleaning_indices = list(range(start_idx, end_idx + 1))
        else:
            cleaning_indices = []
        ...
        print(f"  Before window: indices {before_start} to {start_idx-1} ({len(before_indices)} rows)")
        if include_cleaning_rows:
            print(f"  Cleaning rows: indices {start_idx} to {end_idx} ({len(cleaning_indices)} rows)")
        print(f"  After window: indices {end_idx+1} to {after_end-1} ({len(after_indices)} rows)")
        
        # Combine indices from before, cleaning, and after windows
        window_indices.extend(before_indices)
        window_indices.extend(cleaning_indices)
        ...
        cleaning_event_info.append({
            ...
            'cleaning_rows': len(cleaning_indices),
...
def compute_gaussian_cleaning_probability(df, sigma=15):
    """
    Compute cleaning probability using a Gaussian distribution centered on labeled cleaning events.
    
    Each labeled cleaning event (cleaning_start, cleaning_end) has 100% probability.
    Points between cleaning_start and cleaning_end also have 100% probability.
    Probability decays following a Gaussian distribution as distance increases from cleaning events.
    
    Parameters:
    -----------
    df : DataFrame
        DataFrame with 'label' column containing 'cleaning_start' and 'cleaning_end' labels
    sigma : float
        Standard deviation of the Gaussian distribution (controls decay rate)
        
    Returns:
    --------
    probabilities : numpy array
        Cleaning probability for each row in the dataframe
    """
    print("\n" + "="*80)
    print("COMPUTING GAUSSIAN CLEANING PROBABILITIES")
    print("="*80)
    print(f"\nUsing sigma (std dev) = {sigma}")
    ...
    print(f"Found {len(cleaning_starts)} cleaning_start events")
    print(f"Found {len(cleaning_ends)} cleaning_end events")
    
    # Set 100% probability for labeled cleaning events
    for idx in cleaning_starts + cleaning_ends:
        probabilities[idx] = 1.0
    
    # Process each cleaning event pair
    for start_idx, end_idx in zip(cleaning_starts, cleaning_ends):
        # Set 100% probability for points between start and end
        for idx in range(start_idx, end_idx + 1):
            probabilities[idx] = 1.0
        
        # Apply Gaussian decay before cleaning_start
        for idx in range(0, start_idx):
            distance = start_idx - idx
            gaussian_prob = np.exp(-0.5 * (distance / sigma) ** 2)
            # Keep maximum probability if multiple cleaning events affect this point
            probabilities[idx] = max(probabilities[idx], gaussian_prob)
        
        # Apply Gaussian decay after cleaning_end
        for idx in range(end_idx + 1, n_samples):
            distance = idx - end_idx
            gaussian_prob = np.exp(-0.5 * (distance / sigma) ** 2)
            # Keep maximum probability if multiple cleaning events affect this point
            probabilities[idx] = max(probabilities[idx], gaussian_prob)
    ...
    print(f"  Points with P >= 0.9: {(probabilities >= 0.9).sum()}")
    print(f"  Points with P >= 0.5: {(probabilities >= 0.5).sum()}")
    print(f"  Points with P >= 0.1: {(probabilities >= 0.1).sum()}")
...
def fit_gmm_and_analyze(X_all_scaled, X_cleaning_scaled, df, window_indices, n_clusters=7, random_state=42):
    ...
    # Fit GMM on all data
    gmm = GaussianMixture(n_components=n_clusters, random_state=random_state, n_init=10)
    all_cluster_labels = gmm.fit_predict(X_all_scaled)
    ...
    # Identify "cleaning-associated" clusters
    # Clusters that appear more frequently in cleaning windows
    ...
    return gmm, all_cluster_labels, cleaning_clusters
...
# CONFIGURATION
# ============================================================================
USE_DIFFERENCES = False  # Use raw FFT magnitudes instead of temporal differences
INCLUDE_CLEANING_ROWS = True  # Include cleaning_start to cleaning_end rows in training
...
# Extract window indices around cleaning events (window_size = 45)
window_indices, cleaning_event_info = extract_cleaning_windows(
    df, window_size=43, include_cleaning_rows=INCLUDE_CLEANING_ROWS
)
...
X_all, feature_names = preprocess_data(df, use_differences=USE_DIFFERENCES)
X_all = np.nan_to_num(X_all, nan=0.0, posinf=0.0, neginf=0.0)

print(f"\nFull dataset shape: {X_all.shape}")
print(f"Number of features: {len(feature_names)}")
print(f"Using {'temporal differences' if USE_DIFFERENCES else 'raw FFT magnitudes'}")
...
optimal_k = perform_gmm_clustering(X_all_scaled, n_clusters_range=range(6, 9))

# Fit GMM with 7 clusters (as specified)
gmm, all_cluster_labels, cleaning_clusters = fit_gmm_and_analyze(
...
# Build Autoencoder with updated architecture (encoding_dim=48, hidden_layers=[256, 128])
print("\n" + "="*80)
...
def build_autoencoder(input_dim, encoding_dim, hidden_layers=[256, 128]):
    # Encoder
    input_layer = layers.Input(shape=(input_dim,))
    encoded = layers.Dense(hidden_layers[0], activation='relu')(input_layer)
    encoded = layers.BatchNormalization()(encoded)
    encoded = layers.Dropout(0.2)(encoded)
    encoded = layers.Dense(hidden_layers[1], activation='relu')(encoded)
    encoded = layers.BatchNormalization()(encoded)
    encoded = layers.Dropout(0.2)(encoded)
    encoded = layers.Dense(encoding_dim, activation='relu')(encoded)
    
    # Decoder
    decoded = layers.Dense(hidden_layers[1], activation='relu')(encoded)
    decoded = layers.BatchNormalization()(decoded)
    decoded = layers.Dropout(0.2)(decoded)
    decoded = layers.Dense(hidden_layers[0], activation='relu')(decoded)
    decoded = layers.BatchNormalization()(decoded)
    decoded = layers.Dropout(0.2)(decoded)
...
autoencoder = build_autoencoder(input_dim, encoding_dim, hidden_layers)
...
# ============================================================================
# GAUSSIAN LABEL-BASED PROBABILITY PREDICTION
# ============================================================================

# Compute Gaussian-based cleaning probabilities from labels
# sigma controls how quickly probability decays with distance from cleaning events
gaussian_sigma = 20  # Adjust this to control the spread of the distribution
gaussian_probabilities = compute_gaussian_cleaning_probability(df, sigma=gaussian_sigma)
...
# Calculate threshold (15th percentile as specified)
...
# Calculate cluster-based probabilities
# Higher probability for samples in cleaning-associated clusters
cluster_probabilities = np.zeros(len(all_cluster_labels))
for i, cluster_id in enumerate(all_cluster_labels):
    if cluster_id in cleaning_clusters:
        # Assign high base probability to cleaning clusters
        cluster_probabilities[i] = 0.7
    else:
        # Assign lower base probability to non-cleaning clusters
        cluster_probabilities[i] = 0.3

# Combine both approaches: weighted average (30% autoencoder, 70% GMM clustering)
...
df['gaussian_probability'] = gaussian_probabilities
df['cleaning_probability'] = combined_probabilities
df['predicted_cleaning'] = (combined_probabilities > 0.7).astype(int)
df['gaussian_predicted_cleaning'] = (gaussian_probabilities > 0.5).astype(int)

# Identify most likely cleaning times using Gaussian probability
df_sorted_gaussian = df.sort_values('gaussian_probability', ascending=False)

print("\n" + "="*80)
print("TOP 30 MOST LIKELY CLEANING EVENTS (Gaussian Label-Based)")
print("="*80)

top_gaussian = df_sorted_gaussian.head(30)[['open_start_time', 'close_end_time', 
                                             'cluster', 'gaussian_probability',
                                             'label', 'operational_phase']]

for idx, row in top_gaussian.iterrows():
    rank = list(top_gaussian.index).index(idx) + 1
    print(f"\nRank {rank}:")
    print(f"  Time Period: {row['open_start_time']} to {row['close_end_time']}")
    print(f"  Cluster: {row['cluster']}")
    print(f"  Gaussian Probability: {row['gaussian_probability']:.2%}")
    print(f"  Actual Label: {row['label']}")
    print(f"  Operational Phase: {row['operational_phase']}")

# Identify most likely cleaning times using combined score
...
top_results = df_sorted.head(30)[['open_start_time', 'close_end_time', 
                                    ...
                                    'gaussian_probability', 'cleaning_probability', 
                                    'label', 'operational_phase']]
...
# Evaluate performance - GAUSSIAN LABEL-BASED
print("\n" + "="*80)
print("CLASSIFICATION PERFORMANCE - GAUSSIAN LABEL-BASED")
print("="*80)

from sklearn.metrics import classification_report, confusion_matrix, roc_auc_score, roc_curve

gaussian_predicted = (gaussian_probabilities > 0.5).astype(int)
print("\nClassification Report (Gaussian Label-Based):")
print(classification_report(df['is_cleaning'], gaussian_predicted, 
                            target_names=['Normal', 'Cleaning']))

print("\nConfusion Matrix (Gaussian):")
cm_gaussian = confusion_matrix(df['is_cleaning'], gaussian_predicted)
print(cm_gaussian)

auc_gaussian = roc_auc_score(df['is_cleaning'], gaussian_probabilities)
print(f"\nROC AUC Score (Gaussian): {auc_gaussian:.4f}")
...
fpr_gaussian, tpr_gaussian, _ = roc_curve(df['is_cleaning'], gaussian_probabilities)
...
plt.plot(fpr_gaussian, tpr_gaussian, label=f'Gaussian Label-Based (AUC = {auc_gaussian:.4f})', linewidth=2, color='green')
plt.plot(fpr_ae, tpr_ae, label=f'Autoencoder Only (AUC = {auc_ae:.4f})', linewidth=2, color='blue')
plt.plot(fpr_combined, tpr_combined, label=f'Combined GMM + Autoencoder (AUC = {auc_combined:.4f})', linewidth=2, color='purple')
...
# Plot 1: Gaussian label-based probability (NEW)
ax1 = axes[0]
ax1.fill_between(df.index, 0, gaussian_probabilities, alpha=0.3, color='green', label='Gaussian Probability')
ax1.plot(df.index, gaussian_probabilities, alpha=0.8, color='green', linewidth=1)
ax1.axhline(y=0.5, color='r', linestyle='--', label='Threshold (0.5)')
cleaning_indices = df[df['is_cleaning'] == 1].index
ax1.scatter(cleaning_indices, gaussian_probabilities[cleaning_indices], color='red', 
           s=100, marker='x', label='Actual Cleaning Events', zorder=5)
ax1.set_xlabel('Sample Index')
ax1.set_ylabel('Probability')
ax1.set_title(f'Gaussian Label-Based Cleaning Probability (sigma={gaussian_sigma})', fontsize=14, fontweight='bold')
ax1.legend()
...
ax2 = axes[1]
ax2.plot(df.index, all_mse, alpha=0.7, label='Reconstruction Error')
ax2.axhline(y=threshold, color='r', linestyle='--', label=f'Threshold ({threshold_percentile}th percentile)')
ax2.scatter(cleaning_indices, all_mse[cleaning_indices], color='red', 
           s=100, marker='x', label='Actual Cleaning Events', zorder=5)
ax2.set_xlabel('Sample Index')
ax2.set_ylabel('Reconstruction Error')
ax2.set_title('Reconstruction Error Over Time (Lower = More Similar to Cleaning)')
ax2.legend()
ax2.grid(True, alpha=0.3)

# Plot 3: Autoencoder probability
ax3 = axes[2]
ax3.plot(df.index, ae_probabilities, alpha=0.7, color='blue', label='Autoencoder Probability')
ax3.axhline(y=0.7, color='r', linestyle='--', label='Threshold (0.7)')
ax3.scatter(cleaning_indices, ae_probabilities[cleaning_indices], 
           color='red', s=100, marker='x', label='Actual Cleaning Events', zorder=5)
ax3.set_xlabel('Sample Index')
ax3.set_ylabel('Probability')
ax3.set_title('Autoencoder Cleaning Probability Over Time')
ax3.legend()
ax3.grid(True, alpha=0.3)

# Plot 4: Cluster assignments with probability overlay
ax4 = axes[3]
scatter = ax4.scatter(df.index, all_cluster_labels, c=cluster_probabilities, 
                     cmap='RdYlGn', alpha=0.6, s=30, vmin=0, vmax=1)
ax4.scatter(cleaning_indices, all_cluster_labels[cleaning_indices], 
...
ax4.set_title('Cluster Assignments with Cleaning Probability (Color)')
plt.colorbar(scatter, ax=ax4, label='Cluster Probability')
ax4.legend()
ax4.grid(True, alpha=0.3)

# Plot 5: Combined probability
ax5 = axes[4]
ax5.plot(df.index, combined_probabilities, alpha=0.7, color='purple', label='Combined Probability')
ax5.axhline(y=0.7, color='r', linestyle='--', label='Threshold (0.7)')
ax5.scatter(cleaning_indices, combined_probabilities[cleaning_indices], 
           color='red', s=100, marker='x', label='Actual Cleaning Events', zorder=5)
ax5.set_xlabel('Sample Index')
ax5.set_ylabel('Probability')
ax5.set_title('Combined GMM + Autoencoder Cleaning Probability')
ax5.legend()
...
# Additional visualization: Gaussian probability distribution visualization
fig, axes = plt.subplots(2, 1, figsize=(15, 10))

# Plot 1: Gaussian probability as heatmap-style
ax1 = axes[0]
ax1.imshow([gaussian_probabilities], aspect='auto', cmap='RdYlGn', vmin=0, vmax=1, 
          extent=[0, len(gaussian_probabilities), 0, 1])
# Mark cleaning events
for idx in cleaning_indices:
    ax1.axvline(x=idx, color='black', linestyle='-', alpha=0.5, linewidth=0.5)
ax1.set_xlabel('Sample Index', fontsize=12)
ax1.set_title('Gaussian Cleaning Probability Heatmap (Green=High, Red=Low)', fontsize=14, fontweight='bold')
ax1.set_yticks([])

# Plot 2: Comparison of all probability methods
ax2 = axes[1]
ax2.plot(df.index, gaussian_probabilities, alpha=0.7, color='green', label='Gaussian Label-Based', linewidth=2)
ax2.plot(df.index, ae_probabilities, alpha=0.5, color='blue', label='Autoencoder', linewidth=1)
ax2.plot(df.index, combined_probabilities, alpha=0.5, color='purple', label='Combined', linewidth=1)
ax2.scatter(cleaning_indices, [1.0]*len(cleaning_indices), color='red', 
           s=100, marker='v', label='Actual Cleaning Events', zorder=5)
ax2.set_xlabel('Sample Index', fontsize=12)
ax2.set_ylabel('Probability', fontsize=12)
ax2.set_title('Comparison of Cleaning Probability Methods', fontsize=14, fontweight='bold')
ax2.legend(loc='upper right')
ax2.grid(True, alpha=0.3)
ax2.set_ylim(-0.05, 1.1)

plt.tight_layout()
plt.savefig('probability_methods_comparison.png', dpi=300, bbox_inches='tight')
plt.show()
...
output_df = df[['open_start_time', 'close_end_time', 'cluster', 
                ...
                'gaussian_probability', 'cleaning_probability', 
                'predicted_cleaning', 'gaussian_predicted_cleaning',
                'label', 'operational_phase', 'cycle_count']].copy()

# Save high-probability cleaning events (combined method)
high_prob_cleaning = output_df[output_df['cleaning_probability'] > 0.7].sort_values(
    'cleaning_probability', ascending=False)

# Save high-probability cleaning events (Gaussian method)
high_prob_gaussian = output_df[output_df['gaussian_probability'] > 0.5].sort_values(
    'gaussian_probability', ascending=False)

print(f"\n{len(high_prob_cleaning)} cycles identified with >70% combined probability")
print(f"{len(high_prob_gaussian)} cycles identified with >50% Gaussian probability")
...
high_prob_gaussian.to_csv('cleaning_predictions_gaussian.csv', index=False)

print("\nResults saved to:")
print("  - cleaning_predictions_all_combined.csv")
print("  - cleaning_predictions_high_probability_combined.csv")
print("  - cleaning_predictions_gaussian.csv")
...
print("  - probability_methods_comparison.png")
